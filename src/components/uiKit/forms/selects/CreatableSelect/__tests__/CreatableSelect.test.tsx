...
import { FormCreatableSelectProps, CreatableSelectProps } from "../models";
...
const setup = (props: CreatableSelectProps<{ label: string; value: string }>) => {
    render(<CreatableSelect {...props} />);
};

const formSetup = (props: FormCreatableSelectProps<{ label: string; value: string }>, onSubmit: typeof jest.fn) => {
...
describe("CreatableMultiSelect component", () => {
    ...
    it("Should call onChange when user change selected option", () => {
        ...
        act(() => {
            userEvent.click(screen.getByText(MOCKED_OPTIONS[0].label));
        })

        expect(mockedOnChange).toHaveBeenCalledWith(MOCKED_OPTIONS[0], expect.anything());
    });
...
describe("FormCreatableMultiSelect component", () => {
    ...
    it("Should call onSubmit with selected option", async () => {
        ...
        expect(MOCKED_ON_SUBMIT).toHaveBeenCalledWith({ "my-pretty-select": MOCKED_OPTIONS[0] }, expect.anything());
    });
